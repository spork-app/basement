...
SERVER=$(curl -X POST -H 'Content-type: application/json' -H 'Accept: application/json' -d  "$(SERVER_DATA)" -X POST {{ $route('servers.create-no-credential', ['user_id' => $user]) }})
...
echo "# You can view your configured server at this URL {{ $url('/') }}/$(echo $SERVER | jq -r '.id')"
echo '#                                                                   #'
echo '#####################################################################'
